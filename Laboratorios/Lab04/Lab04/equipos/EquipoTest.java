...
public class EquipoTest{
   ...
   @Test
    ...
    @Test
    public void deberiaLanzarExcepcionSiNoSeConoceUnaPersonaEnEquipo(){
        String [] nombres={"Pedro","Carlos","Juan"};
        Equipo eq= new Equipo(nombres);
        try { 
    ...
    @Test
    public void deberiaLanzarExcepcionSiNoSeCumpleELPorcentaje(){
        String [] nombres={"Garcia","Ospina","Guarin","Pedro","Juan"};
        Equipo eq= new Equipo(nombres);
        try { 
           int valor=eq.valorHoraEstimado();
           fail("No lanza la excepcion");
        } catch (EquipoExcepcion e) {
            assertEquals(EquipoExcepcion.NO_ES_POSIBLE_CALCULAR,e.getMessage());
        }    
    }  
    @Test
    public void deberiaLanzarCalcularValorHoraEstimado() throws EquipoExcepcion{
        String [] nombres={"Pedro","Juan","Santiago","Judas","Ospina"};
        Equipo eq= new Equipo(nombres);
        int val =  eq.valorHoraEstimado();
        assertEquals( val , 150000);
