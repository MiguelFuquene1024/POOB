...
public class POOng implements Serializable {
	...
	private String[][] rSorpresa ={{"FastBall","resources/fasterBall.png"},{"Freezer","resources/freezer.png"},{"Energy","resources/energy.png"},{"Objetivo","resources/objetivo.png"}};
	 ...
	 * */
	public void jugar(){
		if ( rebotaPersonaje() ){getPelota(0).reverseD();}
		getPelota(0).move();
		if(currentSorpresa!=null && chocarSorpresa() ){
			...
			bloque = new Bloque( xPos,250,lastPlayer);
			bloque.setLimit(this);
			bloque.setImage("resources/bloque.png");
		}
		if (bloque!= null ){
			if(!bloque.caer( this ) ){bloque =null;}
		}
		int punto = getPelota(0).getSali(); 
		if ( punto!=-1){
			ronda=( (currentSorpresa== null && bloque == null) )? ronda +1:ronda ;
	...
	public boolean  rebotaPersonaje(){
		if( getPelota(0).rebotaPersonaje(getShapePersonaje( 0)) || getPelota(0).rebotaPersonaje(getShapePersonaje( 1))){
